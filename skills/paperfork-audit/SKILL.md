...
1. Parse user message (paper ID + GitHub URL).
2. POST Convex webhook `/audit` — Convex fans out workers (literature, repo, web, methods).
3. Hermes relays report URL when audit completes.
4. All LLM inference uses Vercel AI Gateway in Convex actions (not Hermes models).
